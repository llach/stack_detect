...
def get_grounding_output(model, image, caption, box_threshold, text_threshold=None, 
    ...
    confidences_sorted, boxes_filt_sorted, pred_phrases_sorted = zip(*zipped_sorted)

    boxes_px = boxes_filt_sorted * np.repeat([[W, H, W, H]], boxes_filt.shape[0], axis=0)
...
if __name__ == "__main__":
    ...
    d = DINOModel(cpu_only=False)
    img = Image.open(f"{os.environ['HOME']}/repos/raw_image.png")


    dino_start = time.time()
    boxes_px, pred_phrases, confidences = d.predict(
        img, 
        "detect all stacks of clothing"
    )
    print(f"DINO took {round(time.time()-dino_start,2)}s")

    image_with_box = plot_boxes_to_image(img.copy(), boxes_px, pred_phrases)[0]
    image_with_box.show()